...
class RoleStore {
  ...
  async getAll(pagedFilterAndSortedRequest: PagedRoleResultRequestDto) {
    this.isLoading = true
    let result = await roleService.getAll(pagedFilterAndSortedRequest).finally(() => (this.isLoading = false))
    this.roles = result
  }

  @action
  async getAllRoles() {
    let result = await roleService.getAll({ keyword: '', skipCount: 0, maxResultCount: 1000 })
