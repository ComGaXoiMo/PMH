...
class LanguageStore {
  ...
  async getAll(pagedFilterAndSortedRequest: PagedLanguageResultRequestDto) {
    this.isLoading = true
    let result = await languageService.getAll(pagedFilterAndSortedRequest).finally(() => this.isLoading = false)
  ...
  async createOrUpdateLanguageText(body) {
    this.isLoading = true
    let result = await languageService.createOrUpdateLanguageText(body).finally(() => this.isLoading = false)
  ...
  async getAllLanguageText(pagedFilterAndSortedRequest: PagedLanguageTextResultRequestDto) {
    this.isLoading = true
    let result = await languageService.getAllLanguageText(pagedFilterAndSortedRequest).finally(() => this.isLoading = false)
