...
class MasterDataStore {
  ...
  @observable haveUserWarning: boolean = false
  constructor() {
  ...
  @action async getUserStatus(userId) {
    ...
    let result = await masterDataService.getUserStatus(userId)
  ...
  @action
  async getAllFeedbackTypes(params: any) {
    params.skipCount = params.skipCount || 0
    params.maxResultCount = params.maxResultCount || 20
    params.moduleId = moduleIds.feedbackType
    this.isLoading = true

    this.feedbackTypeOptions = await wfTrackerService.getAll(params).finally(() => (this.isLoading = false))
  }
  ...
  async filterProductOptions(params: any) {
    ...
    this.productOptions = await masterDataService.filterProductOptions(params).finally(() => (this.isLoading = false))
  }
  ...
  async filterProvinceOptions(params: any) {
    ...
    this.provinceOptions = await masterDataService.filterProvinceOptions(params).finally(() => (this.isLoading = false))
  }
  ...
  async filterDistrictOptions(params: any) {
    ...
    this.districtOptions = await masterDataService.filterDistrictOptions(params).finally(() => (this.isLoading = false))
  }
  ...
  async filterCustomerOptions(params: any) {
    ...
    this.customerOptions = await masterDataService.filterCustomerOptions(params).finally(() => (this.isLoading = false))
  }
  ...
  async filterFeeTypeOptions(params: any) {
    ...
    this.feeTypeOptions = await masterDataService.filterFeeTypeOptions(params).finally(() => (this.isLoading = false))
  }
}
