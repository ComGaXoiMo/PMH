...
class MeetingStore {
  ...
  async create(body: any) {
    let result = await meetingService.create(body);
  ...
  async update(updateStaffInput: any) {
    let result = await meetingService.update(updateStaffInput);
  ...
  @action
  async get(id: number) {
    let result = await meetingService.get(id);
    this.editFinancial = result;
  }
  ...
  async getAll(params: any) {
    this.isLoading = true;
    let result = await meetingService
      .getAll(params)
