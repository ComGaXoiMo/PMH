...
export class ProjectFeeTemplateModel {
  ...
  public static assign(obj) {
    ...
    (abp.localization.languages || []).forEach(language => {
      let templateLanguage = (newObj.notificationTemplates || []).find(template => template.languageName === language.name)
      newObj.templateLanguages[language.name] = TemplateModel.assign(templateLanguage || {languageName: language.name})
    })
    return newObj
  }
}
...
export class ProjectOptionModel {
  ...
  public static assign(obj) {
    ...
    newObj.logoUrl = obj.file?.fileUrl && obj.file?.fileUrl.length ? buildFileUrlWithEncToken(obj.file?.fileUrl) : defaultAvatar
    return newObj
  }
...
export class RowProjectModel extends RowData implements IRowProject{
  ...
  public static assign(obj) {
    ...
    newObj.urlMainPhoto = obj.urlMainPhoto && obj.urlMainPhoto.length > 0
      ? buildFileUrl((obj.urlMainPhoto).replace('//api', '/api'))
      : AppConsts.noImage

    newObj.projectType = (newObj.projectType || []).map(item => ({id: item.propertyTypeId, name: item.typeName}))
    newObj.projectGrade = (newObj.projectGrade || []).map(item => ({id: item.gradeId, name: item.gradeName}))
    newObj.projectFacilities = (newObj.projectFacilities || []).map(item => ({id: item.facilityId, name: item.facilityName}))
    newObj.projectTransportations = (newObj.projectTransportations || []).map(item => ({id: item.transportationId, name: item.transportationName}))
    newObj.projectTenants = (newObj.projectTenants || []).map(item => ({id: item.companyId, name: item.legalName}))
    newObj.projectLandlords = (newObj.projectLandlords || []).map(item => ({id: item.companyId, name: item.legalName}))
    return newObj
  }
...
export class ProjectDetailModel extends RowData implements IRowProject{
  ...
  public static assign(obj) {
    ...
    let newObj = Object.assign(new RowProjectModel(), obj)
    newObj.name = obj.projectName
    newObj.urlMainPhoto = buildFileUrl((obj.urlMainPhoto ? `${AppConfiguration.remoteServiceBaseUrl}${obj.urlMainPhoto}` : AppConsts.noImage)
    .replace('//api', '/api'))
    newObj.projectTypeIds = (newObj.projectType || []).map(item => item.propertyTypeId)
    newObj.gradeIds = (newObj.projectGrade || []).map(item => item.gradeId)
    newObj.facilityIds = (newObj.projectFacility || []).map(item => item.facilityId)
    newObj.transportation = (newObj.projectTransportation || []).map(item => item.transportationId)
    newObj.projectTransportations = (newObj.projectTransportations || []).map(item => ({id: item.transportationId, name: item.transportationName}))
    newObj.projectTenants = (newObj.projectTenants || []).map(item => ({id: item.companyId, name: item.legalName}))
    newObj.projectLandlords = (newObj.projectLandlords || []).map(item => ({id: item.companyId, name: item.legalName}))
    newObj.launchingTime = obj.launchingTime ? moment(obj.launchingTime) : undefined
    newObj.builtDate = obj.builtDate ? moment(obj.builtDate) : undefined
    newObj.yearRenovated = obj.yearRenovated ? moment(obj.yearRenovated) : undefined
    newObj.outOfDate = obj.outOfDate ? moment(obj.outOfDate) : undefined
    newObj.projectAddress = obj.projectAddress && obj.projectAddress.length ? obj.projectAddress[0] : {}
    return newObj
  }
