...
class ForgotPassword extends React.Component<IForgotPasswordProps> {
  ...
  componentDidMount(): void {
    ...
    if (emailAddress && resetCode) {
      this.setState({ forgotPasswordStep: forgotPassword.resetPassword, emailAddress })
  ...
  //         let sendSuccess = await this.props.accountStore!.requestForgotPassword(values)
  //         if (sendSuccess) {
  //           this.setState({ forgotPasswordStep: forgotPassword.resetPassword, emailAddress: values.emailAddress })
  ...
  //         let sendSuccess = await this.props.accountStore!.resetPassword({
  //           ...values,
  ...
  handleSendUsername = async () => {
    ...
    if (formValues.userNameOrEmailAddress) {
      ...
      this.setState({ userNameOrEmailAddress: formValues.userNameOrEmailAddress })
      if (data.state) {
        if (data.state === 2) {
          window.confirmationResult = await firebase
            .auth()
            .signInWithPhoneNumber(formValues.userNameOrEmailAddress, window.recaptchaVerifier)
            .catch((error) => {
        ...
        this.setState({ deviceReceiveConfirmCode: data.state, forgotPasswordStep: forgotPassword.confirmCode })
        notifySuccess('', L(data.message))
      }
    }
  }
  ...
  handleResetPassword = async () => {
    const form = this.formRef.current
    const formValues = await form.validateFields()
    if (this.state.deviceReceiveConfirmCode === 1) {
      await accountService.resetPasswordUser({
        emailAddress: this.state.userNameOrEmailAddress,
        password: formValues.password,
        resetCode: this.state.code
      })
    }
    if (this.state.deviceReceiveConfirmCode === 2) {
      await accountService.PhoneResetPassword({
        userNameOrEmailAddress: this.state.userNameOrEmailAddress,
        password: formValues.password,
        idToken: this.state.idToken
      })
    }
    notifySuccess(L('SUCCESSFULLY'), L('RESET_PASSWORD_SUCCESSFULLY'))
    this.setState({ forgotPasswordStep: forgotPassword.finish })
  }
  ...
  public render() {
    ...
    return (
      <Form ref={this.formRef} validateMessages={validateMessages} layout={'vertical'} className="h-100">
                  ...
                  <div style={{ maxWidth: '360px', margin: 'auto' }}>
                      ...
                      <Row className="mt-3">
                        <Col span={4} className="h-100 text-center">
                          <Button
                            className="rounded"
                            size="large"
                            icon={<LeftOutlined style={{ fontSize: '16px', color: '#d3a429' }} />}
                    ...
                    {forgotPasswordStep === forgotPassword.confirmCode && (
                      <Row className="mt-3">
                        ...
                        <Col span={24} offset={0} className="my-3">
                          <Input6VerifyCode onChange={(code) => this.setState({ code })} />
                        </Col>
                        <Col span={24} className="mt-3 p-1">
                          <Button
                            className="w-100"
                    ...
                    {forgotPasswordStep === forgotPassword.finish && (
                      <Row className="mt-3">
                        <Col span={4} className="h-100 text-center">
                          <Button
                            icon={<LeftOutlined style={{ fontSize: '14px', fontWeight: 600 }} />}
                            className="rounded"
                            onClick={this.backToLogin}
                          />
                        </Col>
