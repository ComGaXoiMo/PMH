...
import { SortAscendingOutlined, CloseOutlined, CheckOutlined } from '@ant-design/icons'
...
@observer
class RatingBadge extends AppComponentListBase<IRatingBadgeProps, IRatingBadgeState> {
  constructor(props) {
    super(props)
    this.state = {
      maxResultCount: 10,
  ...
  openOrCloseModal = (id?) => {
    ...
    this.setState({ visible }, async () => {
      if (visible) {
        id ? await this.props.ratingBadgeStore.getById(id) : await this.props.ratingBadgeStore.createRatingBadge()
        this.setState({ selectedItem: this.props.ratingBadgeStore.editRatingBadge })
      }
    })
  }
  ...
  handleSearch = debounce(async (key, value) => {
    ...
    this.setState({ filters: { ...filters, [key]: value }, skipCount: 0 }, async () => {
      await this.getAll()
    })
  }, 300)
  ...
  handleRowSelect = (selectedIds) => {
    ...
    const disableActivate = items.some(
      (item) => item.isActive && selectedIds.some((selectedId) => selectedId === item.id)
    )
  ...
  renderFilterComponent = () => {
    const { filters } = this.state
    const keywordPlaceHolder = `${this.L('RATING_BADGE_NAME')}, ${this.L('RATING_BADGE_CODE')}`
    return (
      <Row gutter={[8, 8]}>
    ...
    return (
      <>
        {this.isGranted(appPermissions.ratingBadge.update) && (
          <Button shape="circle" className="mr-1" onClick={this.openOrCloseDrawer} icon={<SortAscendingOutlined />} />
        )}
      </>
    )
  }
      ...
      <>
        <DataTable
          ...
          pagination={{
            ...
            total: ratingBadgeStore.pagedData === undefined ? 0 : ratingBadgeStore.pagedData.totalCount,
        ...
        />
        <ActionFooter show={this.isGranted(appPermissions.ratingBadge.delete) && selectedIds.length > 0}>
          <Button
            shape="round"
            size={'large'}
