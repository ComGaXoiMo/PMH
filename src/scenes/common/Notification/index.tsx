...
class Notifications extends AppComponentListBase<INotificationsProps, INotificationsState> {
  ...
  getAll = async () => {
    ...
    this.setState({loading: false, notifications: result.items, totalCount: result.totalCount})
  }
    ...
    const columns = [
      {
        ...
        width: 100,
        render: (icon) => <div>
          {icon ? <Icon component={icon} className="notification-icon" style={{opacity: .5}}/> : null}
        </div>
      },
      {
