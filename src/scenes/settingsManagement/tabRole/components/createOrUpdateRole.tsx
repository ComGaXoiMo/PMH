...
class CreateOrUpdateRole extends React.Component<ICreateOrUpdateRoleProps> {
  ...
  componentDidUpdate(prevProps: Readonly<ICreateOrUpdateRoleProps>, prevState: Readonly<{}>, snapshot?: any): void {
    if (!prevProps.visible && this.props.visible) {
      const permissionAdmin = this.props.permissions.filter(permission => permission.name.startsWith('PagesAdministration.'))
  ...
  handleGroupPermission(permissions) {
    ...
    let groupPermissions = Object.keys(groupPermissionObject).reduce((groupPermissions, key) => {
      ...
      const selectedPermissions = groupPermissionObject[key].filter(permission =>
  ...
  render() {
    ...
    return (
      <Modal
        ...
        width={800}
      >
        <Form ref={this.props.formRef} validateMessages={validateMessages} layout={'vertical'}
              size="large">
          <Tabs defaultActiveKey={'role'} size={'small'} tabBarGutter={64}>
            <TabPane tab={L('ST_ROLE_INFO')} key={'role'}>
              ...
              <Form.Item label={L('ST_ROLE_DISPLAY_NAME')} {...formVerticalLayout} name="displayName"
