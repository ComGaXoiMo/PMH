...
class ReceiptService {
  ...
  public async filter(params: any): Promise<PagedResultDto<ReceiptModel>> {
    if (!params) {
      params = {}
    }

    params.transactionType = cashAdvanceTransactionTypes.receipt
    ...
    result.items = ReceiptModel.assigns(result.items)
    return result
  }
