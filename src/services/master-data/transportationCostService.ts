...
class TransportationCostService {
  public async create(body: any) {
    let res = await http.post('api/services/app/Transportation/Create', body)
  ...
  public async filter(params: any): Promise<PagedResultDto<any>> {
    let res = await http.get('api/services/app/Transportation/GetAll', {
      params
    })
  ...
  public async update(body: any) {
    let res = await http.put('api/services/app/Transportation/Update', body)
    notifySuccess(LNotification('SUCCESS'), LNotification(L('SAVE_SUCCESSFULLY')))
    return TransportationCostModel.assign(res.data.result)
  }
  ...
  public async getById(id): Promise<any> {
    ...
    let result = await http.get('/api/services/app/Transportation/Get', {
      params: { id }
    })
  ...
  public async activateOrDeactivate(ids: number, isActive) {
    let response = await http.post('api/services/app/Transportation/MultiActives', ids, { params: { isActive } })
    notifySuccess(LNotification('SUCCESS'), LNotification('SAVING_SUCCESSFULLY'))
    return response.data
  }

  public async delete(id: number, isActive) {
    let response = await http.delete('api/services/app/Transportation/Delete', {
      params: { isActive }
    })
    return response.data
  }
  ...
  public async downloadTemplateImport() {
    let response = await http.get('api/Imports/Transportation/GetTemplate', { responseType: 'blob' })
  ...
  public async importByTemplateImport(file) {
    ...
    let response = await http.post('api/Imports/Transportation/ImportFromExcel', formData, config)
    return response.data
  }

  public async exportTransportationCost(params: any): Promise<any> {
    let res = await http.get('api/Export/ExportTransportation', {
      params,
      responseType: 'blob'
    })
