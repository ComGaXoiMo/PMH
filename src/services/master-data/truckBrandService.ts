...
class TruckBrandService {
  public async create(body: any) {
    let res = await http.post('api/services/app/TruckBrands/Create', body)
  ...
  public async filter(params: any): Promise<PagedResultDto<any>> {
    let res = await http.get('api/services/app/TruckBrands/GetAll', { params })
  ...
  public async getAll(params: any): Promise<TruckBrandModel[]> {
    let res = await http.get('api/services/app/TruckBrands/GetLists', { params })
    const result = res.data.result
    result.items = TruckBrandModel.assigns(result.items || [])
    return result
  }
  ...
  public async update(body: any) {
    let res = await http.put('api/services/app/TruckBrands/Update', body)
    notifySuccess(LNotification('SUCCESS'), LNotification(L('SAVE_SUCCESSFULLY')))
    return TruckBrandModel.assign(res.data.result)
  }

  public async updateSortList(body: any) {
    let res = await http.post('api/services/app/TruckBrands/SortList', body)
  ...
  public async getById(id): Promise<any> {
    ...
    let result = await http.get('api/services/app/TruckBrands/Get', { params: { id } })
    return TruckBrandModel.assign(result.data.result)
  }

  public async activateOrDeactivate(ids: number[], isActive) {
    let response = await http.post('api/services/app/TruckBrands/MultiActives', ids, { params: { isActive } })
    notifySuccess(LNotification('SUCCESS'), LNotification('SAVING_SUCCESSFULLY'))
    return response.data
  }

  public async delete(id: number, isActive) {
    let response = await http.delete('api/services/app/TruckBrands/Delete', { params: { id, isActive } })
    return response.data
  }

  public async downloadTemplateImport() {
    let response = await http.get('api/Imports/TruckBrands​/GetTemplateImport', { responseType: 'blob' })
  ...
  public async importByTemplateImport(file) {
    ...
    let response = await http.post('api/Imports/TruckBrands​/ImportFromExcel', formData, config)
    return response.data
  }

  public async exportTruckBrand(params: any): Promise<any> {
    let res = await http.get('/api/Export/ExportTruckBrand', { params, responseType: 'blob' })
