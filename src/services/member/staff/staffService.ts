...
class StaffService {
  ...
  public async get(id: number): Promise<any> {
    ...
    let res = await http.get('api/services/app/Employees/Get', { params: { id } })
    if (res.data.result && res.data.result.birthDate) {
      res.data.result.birthDate = moment(res.data.result.birthDate)
    }
    return res.data.result
  ...
  public async getProjectRoles(params: any): Promise<any> {
    let res = await http.get('api/services/app/Employees/GetProjectRoles', { params })
    return res.data.result
  }

  public async setProjectRole(body: any) {
    let res = await http.post('api/services/app/Employees/SetProjectRoles', body)
    return res.data.result
  }
