...
export const createBarHorizontalChart = (
  ...
  let chart = am4core.create(elementChartId, am4charts.XYChart)
  chart.logo.hidden = true
  ...
  chart.events.on('datavalidated', (ev) => {
    ...
    let numberSeries = chart.data.length ? chart.data[0].numberSeries : 0

    // Calculate how we need to adjust chart height
    let adjustHeight = chart.data.length * cellSize * (stacked ? 1 : numberSeries) - (categoryAxis?.pixelHeight || 0)
