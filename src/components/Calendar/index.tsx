...
export const CustomEvent = (props) => {
  ...
  const itemClass = `text-center ant-tag booking-slot ${event.isAvailable ? 'available' : 'unavailable'}`
  return (
...
export const ToolbarCalendar = (props) => {
  ...
  let previousYear = moment(date).subtract(1, 'years').toDate()
  let nextYear = moment(date).add(1, 'years').toDate()
