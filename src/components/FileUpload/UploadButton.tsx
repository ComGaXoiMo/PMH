...
class UploadButton extends AppComponentBase<IUploadButtonProps> {
  ...
  handleBeforeUploadFile = (file, fileList) => {
    ...
    if (!this.validateFile(file)) {
      message.warning(
        LError("MAX_FILE_SIZE_UPLOAD_{0}_MB", this.props.maxSize)
      );
      return false;
    }
    // Validate file type
  ...
  onChange = async (info) => {
    ...
    switch (this.props.moduleId) {
      case moduleIds.unit: {
        ...
        message.success(
          LNotification(
            "UPLOAD_SUCCESS_WE_WILL_INFORM_YOU_ONCE_PROGRESSION_IS_DONE"
          )
        );
        break;
      }
    ...
    if (this.props.onSelectFile && fileList instanceof Array) {
      ...
      let limitFiles = !this.props.maxNumberFile
        ? fileList
        : tempFileList.slice(-this.props.maxNumberFile);
      ...
      if (
        lastFile.uid === file.uid &&
        !!this.props.maxNumberFile &&
        fileList.length > this.props.maxNumberFile
      ) {
        message.warning(
          LNotification(
            "LIMITED_MAXIMUM_{0}_FILE_CAN_UPLOAD",
            this.props.maxNumberFile
          )
        );
      }
  ...
  render() {
    const { multiple, wrapClass, disabled, toolTipTex, label, icon } =
      this.props;
