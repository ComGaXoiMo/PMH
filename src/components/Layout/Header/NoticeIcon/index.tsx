...
const NoticeIcon: React.FC<NoticeIconProps> & {
      ...
      <>
        <Tabs onChange={onTabChange}>
          {([children] || []).map((child: any) => {
            ...
            const { data, title, count, tabKey, showClear, showViewMore, emptyText, icon } = child.props
  ...
  const NoticeBellIcon = bell || <BellOutlined className={styles.icon} style={{ fontSize: '1.4rem', color: 'white' }} />
  const haveUnreadMessage = count > 0
  const badgeProps = haveUnreadMessage ? { status: 'processing' as const } : ({ dot: false } as BadgeProps)
  const trigger = (
